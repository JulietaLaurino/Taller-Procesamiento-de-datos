...
## **Guardado**
Después de tanto amasar los datos, llegó el momento de guardarlos. Para guardarlo como un archivo excel usamos el paquete `xlsx` que instalamos al principio de todo. Primero va el nombre de la tabla que quiero guardar como archivo de excel, y luego, el que quiero que tenga ese archivo. Pueden guardarse tablas en un mismo archivo y diferentes hojas usando los argumentos `sheetName = "nombre de la hoja"` y `append = TRUE`
```{r}
library(xlsx)
write.xlsx(data, file= "PRIMINGdata.gorilla.xlsx")
```

Por supuesto que también podría guardarse un archivo csv o bien podría no guardar nada y seguir con el análisis en este mismo script (que es lo que hago en general). 
